...
import os
from utils import reg_eval_model, load_checkpoint, cond_eval_model
from dataloader import reg_dataloader, cond_dataloader
import argparse
...
from models.Amygdala_lowroad_attention_Final import Amygdala_lowroad as Amygdala_lowroad_attention
from models.Amygdala_lowroad_attention_Ishighroadonly_gist_model6v2 import Amygdala_lowroad as Amygdala_lowroad_gist_model6v2
...
parser.add_argument('--TRAIN', default='IAPS_Conditioning_train', type=str, help='the folder of training data')
parser.add_argument('--VAL', default='IAPS_Conditioning_val', type=str, help='the folder of validation data')
parser.add_argument('--TEST', default='IAPS_10-10-80_test2', type=str, help='the folder of test data')

parser.add_argument('--csv_train', default='/data/seowung/data/IAPS_Conditioning_train.csv', type=str, help='the path of training data csv file')
parser.add_argument('--csv_val', default='/data/seowung/data/IAPS_Conditioning_val.csv', type=str, help='the path of training data csv file')
parser.add_argument('--csv_test', default='/data/seowung/data/IAPS_10-10-80_test2.csv',
                    type=str, help='the path of training data csv file')

parser.add_argument('--batch_size', default=10, type=int, help='batch size')

parser.add_argument('--model_dir', default='/home/seowung/Brain inspired AI_Fear Conditioning/code/savedmodel', type=str, help='where to save the trained model')
parser.add_argument('--model_name', default='Amy_IntermediateRoadAttention_lr1e5_stepsize30_bs16_ep100_FinetuningIAPS_10-10-80_2_Feb242022_epoch96_quadrant_epoch50_conditioned_epoch1.pth',
...
parser.add_argument('--n_layers', default=3, type=int, help='the number of layers in Amygdala; this setting is only for model_to_run of 1')
...
parser.add_argument('--is_gist', default=False, type=lambda x: (str(x).lower() == 'true'), help='whether to apply gist as input to low-road instead')
        ...
        with open(path_result_csv, 'w', newline='') as file:
            ...
            for file_name in sorted(file_list):
                ...
                for i in range(num_datasets):
                    ...
                    dataloaders, dataset_sizes = cond_dataloader(args.data_dir, args.TRAIN, args.VAL, dataset,
                    ...
                    # ----------------test-----------------------------#
                    score_c_r = cond_eval_model(dataloaders, dataset_sizes, dataset, model, criterion, device,is_gist=is_gist, is_saliency=is_saliency)
