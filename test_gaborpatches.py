...
r"""Test how decoded valence for a whole set of Gabor patches changes across conditioning.

Loads the --initial_model_name and --conditioned_model_name checkpoints from `model_dir`,
runs both over every Gabor patch found in `data_dir` (varying orientation/frequency/
contrast), and writes one CSV (test_gaborpatches.csv) of (model, gabor, valence) rows.
Unlike test_gaborpatch_iteration.py (single patch, many checkpoints), this compares two
checkpoints against many patches -- e.g. to see whether valence generalizes smoothly
across nearby orientations after conditioning on just 45/135 degrees.
"""
...
def iter_run_allmodels(dir, modelName):
    """Print every checkpoint filename under `dir` matching `modelName`'s prefix (unused helper)."""
...
if __name__ == '__main__':
    ...
    if len(file_list) < 1:
        ...
        if not os.path.exists(args.csv_result_recording_folder):
            os.makedirs(args.csv_result_recording_folder, exist_ok=True)
        ...
        result_df.to_csv(args.csv_result_recording_folder + "test_gaborpatches" + '.csv')
