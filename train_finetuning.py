...
from models.VGG_Model import VGG_Unfreeze_All,VGG_Freeze_conv, VGG_BN_Freeze_conv, VGG_Freeze_conv_FC1, VGG_Freeze_conv_FC2, Visual_Cortex_Amygdala
...
r"""This code trains the regression model for finetuning the pretrained model for conditioning. In parser.add_argument, 
    ...
    fine-tuning code is the quadrant dimension. Based on the pre-defined location from the dataloader, the affective 
    pictures are in the 4th quadrant. If you want to change the position of the image, you need to access the dataloader
    in the 'dataloader.py' and change the position manually.
...
parser.add_argument('--lr', default=1e-5, type=float, help='initial learning rate for SGD')
...
parser.add_argument('--model_name',
                    default='vca_IAPS_quadrant_batch16_lr1e-5.pth',
...
parser.add_argument('--file_name',
                    default='vca_IAPS_batch10_lr2e-4_epoch23.pth',
                    type=str, help='name of the trained model')
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for phase in [TRAIN, VAL]:
            ...
            if phase == VAL and epoch_loss < best_loss: # loss or R
                ...
                save_checkpoint(best_R, best_loss, best_epoch, model, best_model_wts,
                                optimizer, os.path.join(args.model_dir, args.model_name[:-4]
                                                        +'.pth'))
...
def main():
    """Load or build a model (resume / fine-tune from --file_name / fresh), fine-tune it
    on the quadrant-cropped US-image dataset (US confined to one quadrant, matching the
    input layout later used during conditioning), and evaluate before and after training.
    """
    free_gpu_cache()
    args = parser.parse_args()
    ...
    dataloaders_test, dataset_sizes_test = quadrant_finetune_dataloader(args.data_dir, args.TRAIN, args.VAL, args.TEST,
    ...
    dataloaders_train, dataset_sizes_train = quadrant_finetune_dataloader(args.data_dir,
